...
SSL_CTX *dtls_ctx_init(int verify_mode, ssl_verify_cb *cb, const tlscfg *cfg) {
  ...
  if (!str_isempty(cfg->cafile) || !str_isempty(cfg->capath)) {
    if (!SSL_CTX_load_verify_locations(ctx, str_nullforempty(cfg->cafile),
                                       str_nullforempty(cfg->capath))) {
      SSL_CTX_free(ctx);
      return NULL;
    }
  }
...
ptrdiff_t dtls_sess_send_pending(dtls_sess *sess, void *carrier,
                                 const void *dest, int destlen) {
  if (sess->ssl == NULL) {
    return -2;
  }
  ...
  if (pending > 0) {
    ...
    if (sess->sink->sched != NULL) {
      ...
      sess->sink->sched(carrier, dtls_sess_get_timeout(sess, &tv) ? &tv : NULL);
    }
...
ptrdiff_t dtls_sess_put_packet(dtls_sess *sess, void *carrier, const void *buf,
...
ptrdiff_t dtls_sess_handle_timeout(dtls_sess *sess, void *carrier,
                                   const void *dest, int destlen) {
  if (!SSL_is_init_finished(sess->ssl)) {
