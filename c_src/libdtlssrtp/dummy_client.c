...
static int sleep_for(long long miliseconds) {
  struct timespec t = {.tv_sec = miliseconds / 1000,
...
int run(int argc, char **argv) {
  for (int i = 0; i < RTP_PACKET_LEN; ++i) {
    dummy_message[i] = 'd';
  }
  ...
  const char *bindaddr = NULL, *peeraddr = NULL, *certfile = NULL,
             *pkeyfile = NULL;
  ...
  do {
    ...
    do {
      ...
      do {
        ...
        for (int i = 0; i < 10; ++i) {
          sleep_for(10);
          sendto(fd, dummy_message, RTP_PACKET_LEN, 0,
                 (struct sockaddr *)&raddr.s4, sizeof(raddr.s4));
        }
